...
def render(total_direct_cost_local, program_duration, currency_choice, currency_config, currency_symbol, exchange_rate):
    ...
    with left_col:
        with st.container(border=True):
            ...
            with top_c1: loan_principal_local = st.slider(f"Loan Principal Borrowed ({currency_symbol}):", min_value=0.0, max_value=max_loan_amount, value=min(max_loan_amount, float(total_direct_cost_local * 0.7)), step=float(currency_config["tuition"]["step"]), help="The stated amount you borrow for your degree.")
            with top_c2: custom_monthly_payment = st.number_input(f"Custom Monthly Payment (Optional):", min_value=0, value=0, step=100, help="Want to pay less each month? Enter a lower custom payment to simulate Income-Driven Repayment. Leave at 0 for standard 10-year rules.")
            ...
            with col_set2: origination_fee = st.slider("Loan Origination Fee (%):", min_value=0.0, max_value=10.0, value=1.5, step=0.1, help="An upfront fee charged by the lender just to process the loan.")
            
            grace_period_months = st.slider("Months to Find a Job (Post-Grad):", min_value=0, max_value=12, value=6, step=1, help="Time spent job hunting after graduation. Standard grace periods are 6 months, but interest still compounds while your income is $0!")
    ...
    with left_col:
        ...
        with col_cap1: 
            st.metric(label=f"Target Funding Needed ({currency_symbol})", value=f"{currency_symbol}{loan_principal_local:,.0f}")
            st.metric(label=f"Required Loan Amount ({currency_symbol})", value=f"{currency_symbol}{gross_loan_local:,.0f}", delta=f"+{currency_symbol}{origination_fee_local:,.0f} (Processing Fee)", delta_color="inverse")
        with col_cap2: 
            st.metric(label=f"Actual Starting Debt ({currency_symbol})", value=f"{currency_symbol}{capitalized_principal_local:,.0f}", delta=f"+{currency_symbol}{moratorium_interest_local:,.0f} (Study Interest)", delta_color="inverse", help=f"Capitalized Principal: This is your TRUE starting loan balance. It includes the original loan PLUS all the interest that secretly accumulated during your {int(program_duration*12) + grace_period_months} months of studying and job hunting.")
        ...
        if grace_period_months >= 6 and job_hunt_penalty_usd > 0:
            st.error(f"**Job Hunt Delay Alert:** Taking {grace_period_months} months to find a job adds **{currency_symbol}{job_hunt_penalty_local:,.0f}** in extra interest to your debt while you are unemployed!")
    ...
    with right_col:
        ...
        if custom_monthly_payment > 0 and custom_monthly_payment_usd < monthly_interest_accrual_usd:
            st.error(f"**Danger - Debt Will Grow Forever:** Your custom payment of **{currency_symbol}{custom_monthly_payment:,.0f}** is smaller than the **{currency_symbol}{monthly_interest_accrual_local:,.0f}** in interest charged each month. Your loan will never be paid off!")
        ...
        with col_amt1: st.metric(label=f"{payment_label} ({currency_symbol})", value=f"{currency_symbol}{display_payment_usd * exchange_rate:,.0f}", help="Your expected monthly cash outflow. Standard assumes a rigid 10-year term.")
        with col_amt2: st.metric(label=f"Total Interest Paid to Bank ({currency_symbol})", value=f"{currency_symbol}{total_lifetime_interest_usd * exchange_rate:,.0f}", help="The total amount of pure interest the bank makes off of you.")
